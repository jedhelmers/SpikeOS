...
The `run concurrent` command demonstrates mutex-protected shared state between two threads. `run tetris` opens a framebuffer window for the game. The boot splash runs during kernel init (before interrupts are enabled) using busy-wait timing.
